...
namespace MvcWebCrawler.Controllers
{
    public class RevenueController : Controller
    {
        ...
        // GET: Revenue/Favorites - 只顯示我的最愛公司
        public async Task<ActionResult> Favorites()
        {
            ...
            // 只篩選出收藏的公司
            var favoriteData = allRevenueData
                .Where(x => favoriteIds.Contains(x.公司代號))
                .OrderBy(x => x.公司代號)
                .ToList();
            
            System.Diagnostics.Debug.WriteLine($"Favorites view: Total favorites: {favoriteIds.Count}, Displayed: {favoriteData.Count}");
            
            ViewBag.IsFavoritesView = true; // 標記這是我的最愛頁面
            
            return View(favoriteData);
        }

        // GET: Revenue
        public async Task<ActionResult> Index(string industry = "", string searchCompanyId = "")
        {
            var allRevenueData = await GetMonthlyRevenueAsync();
            ...
            // 如果沒有指定產業，預設選擇「建材營造」
            if (string.IsNullOrEmpty(industry) && string.IsNullOrEmpty(searchCompanyId) && constructionIndustry != null)
            {
            ...
            ViewBag.SearchCompanyId = searchCompanyId;
            
            // 根據選擇的產業別進行篩選
            var filteredData = allRevenueData;
            
            // 優先使用股票代碼搜尋
            if (!string.IsNullOrEmpty(searchCompanyId))
            {
                filteredData = filteredData
                    .Where(x => x.公司代號.Contains(searchCompanyId.Trim()) || 
                                x.公司名稱.Contains(searchCompanyId.Trim()))
                    .ToList();
                
                System.Diagnostics.Debug.WriteLine($"Search by company ID/Name: {searchCompanyId}, Results: {filteredData.Count}");
            }
            else if (!string.IsNullOrEmpty(industry))
            {
                // 如果沒有搜尋，則使用產業別篩選
                filteredData = filteredData.Where(x => x.產業別 == industry).ToList();
            }
        ...
        // 比較兩個數字，返回顏色類別
        public static string GetComparisonColor(string value1, string value2)
        {
            if (string.IsNullOrEmpty(value1) || string.IsNullOrEmpty(value2))
                return "";

            if (decimal.TryParse(value1, out decimal num1) && decimal.TryParse(value2, out decimal num2))
            {
                if (num1 > num2)
                    return "text-success fw-bold"; // 綠色粗體 - 較大
                else if (num1 < num2)
                    return "text-danger fw-bold"; // 紅色粗體 - 較小
                else
                    return ""; // 相等 - 無特殊顏色
            }
            return "";
